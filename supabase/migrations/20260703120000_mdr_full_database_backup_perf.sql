-- Speed up full database backup RPC: avoid expensive row ordering and skip temp backup tables.
...
begin
  ...
  for v_table in
    ...
    where n.nspname = 'public'
      ...
      and c.relname !~* '(^|_)(bak|backup|dup_bak)(_|$)'
    order by c.relname
  loop
    execute format(
      'select coalesce(jsonb_agg(to_jsonb(t)), ''[]''::jsonb), count(*) from %I.%I t',
      'public',
      v_table.table_name
    )
    into v_rows, v_count;
